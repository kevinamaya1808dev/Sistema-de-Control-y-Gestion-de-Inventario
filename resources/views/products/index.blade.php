...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6" x-data="productManagement()">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 pb-6 border-b border-slate-100 dark:border-slate-800/80">
        ...
        <button @click="openCreateModal()" 
                class="w-full sm:w-auto group relative px-6 py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black rounded-xl shadow-lg shadow-indigo-600/20 hover:shadow-indigo-600/30 transition-all flex items-center justify-center gap-2.5 cursor-pointer transform hover:-translate-y-0.5 active:translate-y-0">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200/70 dark:border-slate-800/80 overflow-hidden">
        ...
        <div class="px-5 sm:px-7 py-5 bg-slate-50/50 dark:bg-slate-950/40 border-b border-slate-100 dark:border-slate-800/80">
        ...
        <div class="block md:hidden divide-y divide-slate-100 dark:divide-slate-800/60">
            ...
            <div class="p-4 sm:p-5 flex flex-col space-y-4 hover:bg-indigo-50/10 dark:hover:bg-slate-800/20 transition-colors">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-400 dark:text-slate-500 tabular-nums">#{{ $product->id }}</span>
                    <span class="font-black px-2.5 py-1 bg-slate-100 dark:bg-slate-800 border border-slate-200/50 dark:border-slate-700 rounded-lg text-[10px] tracking-wide text-slate-800 dark:text-slate-200">
                        {{ $product->category->name ?? 'Sin Categoría' }}
                    </span>
                </div>

                <div class="flex items-center gap-3.5">
                    <div class="w-14 h-14 shrink-0 rounded-xl overflow-hidden border border-slate-200/60 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 shadow-xs flex items-center justify-center">
                        @if($product->image)
                ...
                <div class="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/60">
                    ...
                    <div class="flex items-center gap-1.5">
                        <button @click="openEditModal({{ $product->id }}, '{{ addslashes($product->name) }}', '{{ $product->category_id ?? '' }}', {{ $product->price }}, {{ $product->stock }}, '{{ $product->sku ?? '' }}')" 
                                class="p-2.5 text-amber-500 hover:text-amber-600 hover:bg-amber-500/10 border border-slate-200/60 dark:border-slate-800 rounded-xl transition-all cursor-pointer inline-flex items-center shadow-xs" title="Modificar Producto">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                        </button>

                        <button @click="openDeleteModal({{ $product->id }})" 
                                class="p-2.5 text-rose-500 hover:text-rose-600 hover:bg-rose-500/10 border border-slate-200/60 dark:border-slate-800 rounded-xl transition-all cursor-pointer inline-flex items-center shadow-xs" title="Remover del Servidor">
            ...
            @empty
            <div class="p-10 text-center text-slate-400 dark:text-slate-600 font-bold tracking-wide">
                <div class="flex flex-col items-center justify-center space-y-3">
                    ...
                    <span class="text-xs uppercase tracking-wider">No hay productos registrados.</span>
                </div>
            </div>
            @endforelse
        </div>
    ...
    <div x-show="modals.create" class="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-md flex items-center justify-center p-4" x-transition style="display: none;">
        <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200/40 dark:border-slate-800" @click.outside="closeModal('create')">
            <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-950/20">
                ...
                <button type="button" @click="closeModal('create')" class="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 flex items-center justify-center cursor-pointer font-bold">✕</button>
            </div>
            <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <!-- ERRORES DE VALIDACIÓN (CREAR) -->
                @if ($errors->any())
                    <div class="mx-6 mt-4 p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl text-rose-600 dark:text-rose-400 text-xs font-bold space-y-1">
                        <p class="uppercase font-black tracking-wider">⚠️ Revisa los siguientes campos:</p>
                ...
                <div class="p-5 sm:p-6 space-y-4 sm:space-y-5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" step="0.01" name="price" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:text-white text-xs font-bold focus:outline-hidden" required>
                        </div>
                        <div>
                            ...
                            <input type="number" name="stock" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:text-white text-xs font-bold focus:outline-hidden" required>
                        </div>
                        <div>
                            ...
                            <input type="text" name="sku" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:text-white text-xs font-bold focus:outline-hidden uppercase" required>
                        </div>
                    </div>
                    <div>
    ...
    <div x-show="modals.edit" class="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-md flex items-center justify-center p-4" x-transition style="display: none;">
        <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200/40 dark:border-slate-800" @click.outside="closeModal('edit')">
            <div class="px-6 py-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-950/20">
                ...
                <button type="button" @click="closeModal('edit')" class="w-8 h-8 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 flex items-center justify-center cursor-pointer font-bold">✕</button>
            </div>
            <form :action="'{{ route('products.index') }}/' + formEdit.id" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')

                <!-- ERRORES DE VALIDACIÓN (EDITAR) -->
                @if ($errors->any())
                    <div class="mx-6 mt-4 p-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl text-rose-600 dark:text-rose-400 text-xs font-bold space-y-1">
                        <p class="uppercase font-black tracking-wider">⚠️ Revisa los siguientes campos:</p>
                        <ul class="list-disc pl-4 space-y-0.5">
                ...
                <div class="p-5 sm:p-6 space-y-4 sm:space-y-5">
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Nombre del Producto</label>
                        <input type="text" x-model="formEdit.name" name="name" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:text-white text-xs font-bold focus:outline-hidden" required>
                    </div>
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Categoría</label>
                        <select x-model="formEdit.category_id" name="category_id" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:text-white text-xs font-bold focus:outline-hidden cursor-pointer" required>
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Precio Venta</label>
                            <input type="number" step="0.01" x-model="formEdit.price" name="price" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:text-white text-xs font-bold focus:outline-hidden" required>
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Stock</label>
                            <input type="number" x-model="formEdit.stock" name="stock" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:text-white text-xs font-bold focus:outline-hidden" required>
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Código SKU</label>
                            <input type="text" x-model="formEdit.sku" name="sku" class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:text-white text-xs font-bold focus:outline-hidden uppercase" required>
                        </div>
                    </div>
                    <div>
