...
@section('content')
<x-app-container>
<div class="max-w-7xl mx-auto space-y-4 sm:space-y-6 w-full min-w-0 max-w-full overflow-x-hidden" x-data="productManagement()">
    ...
    {{-- CONTENEDOR MAESTRO --}}
    <div class="table-container w-full min-w-0 max-w-full overflow-hidden">
        <div class="px-5 py-4 sm:px-6 sm:py-5 bg-slate-100/60 dark:bg-[#070a11]/80 border-b border-slate-200/80 dark:border-slate-800/80 flex items-center justify-between">
            <div>
        ...
        <div class="block md:hidden p-3 space-y-3 bg-slate-50/50 dark:bg-[#080c16]/50 w-full min-w-0 box-border">
            ...
            <div class="p-4 rounded-2xl bg-white dark:bg-[#0d121f] border border-slate-200/80 dark:border-slate-800/80 space-y-3 shadow-sm w-full min-w-0 box-border" x-data="{ openSizes: false }">
                <div class="flex items-start justify-between gap-2 w-full min-w-0">
                    <div class="flex items-center gap-2.5 min-w-0 flex-1">
                        <div class="w-11 h-11 rounded-xl overflow-hidden border border-slate-200/80 dark:border-slate-800/80 bg-slate-100 dark:bg-slate-900 flex items-center justify-center shrink-0">
                            @if($product->image)
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <p class="text-[10px] text-slate-400 font-mono truncate">SKU: {{ $product->sku ?? 'N/A' }}</p>
                            <span class="mt-0.5 font-black px-2 py-0.5 bg-slate-100 dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-800 rounded-lg text-[9px] text-slate-700 dark:text-slate-300 inline-block truncate max-w-full">
                    ...
                    <div class="flex items-center gap-1 shrink-0">
                        <button @click="openEditModal({{ $product->id }}, '{{ addslashes($product->name) }}', '{{ $product->category_id ?? '' }}', {{ $product->price }}, {{ $product->stock }}, '{{ $product->sku ?? '' }}', '{{ $product->image ? route('products.image', $product->image) : '' }}')"
                            class="p-1.5 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-500/10 rounded-xl transition-all cursor-pointer">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                        </button>
                        <button @click="openDeleteModal({{ $product->id }})" class="p-1.5 text-rose-500 hover:bg-rose-500/10 rounded-xl transition-all cursor-pointer">
                ...
                <div class="pt-2 border-t border-slate-100 dark:border-slate-800/60 w-full">
                    <button @click="openSizes = !openSizes" type="button" class="w-full flex items-center justify-between px-3 py-1.5 bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 rounded-xl text-xs font-bold transition-all">
                        <span>Ver {{ $product->sizes->where('stock', '>', 0)->count() }} Tallas Disponibles</span>
                        <div class="flex items-center gap-1">
                            <span class="text-[10px] text-slate-400">({{ $product->stock }} u.)</span>
                    ...
                    <div x-show="openSizes" x-transition class="mt-2 grid grid-cols-2 gap-1.5 p-2 bg-slate-900/60 rounded-xl border border-slate-800">
                        ...
                        <div class="flex justify-between items-center px-2 py-1 bg-slate-800/40 rounded-lg text-[11px]">
                            <span class="font-bold text-slate-300">Talla {{ $size->talla }}</span>
                            <span class="font-black text-indigo-400">{{ $size->stock }} u.</span>
                        </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/60 text-xs w-full">
                    <div>
            ...
            @empty
            <div class="p-8 text-center text-slate-400 dark:text-slate-600 font-bold uppercase text-xs">No hay productos registrados.</div>
            @endforelse
        </div>
        ...
        <div class="hidden md:block overflow-x-auto w-full">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr>
                        <th class="table-th w-14">ID</th>
                        <th class="table-th w-14">Img</th>
                        <th class="table-th">Producto / SKU</th>
                        <th class="table-th w-28">Categoría</th>
                        <th class="table-th w-44">Tallas</th>
                        <th class="table-th w-24">Precio</th>
                        <th class="table-th w-20">Total</th>
                        <th class="table-th text-right w-24">Acciones</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                    @forelse($products as $product)
                    <tr class="table-tr hover:bg-indigo-50/30 dark:hover:bg-indigo-500/5 transition-colors">
                        <td class="table-td font-bold text-slate-400 dark:text-slate-500 tabular-nums">#{{ $product->id }}</td>
                        <td class="table-td">
                            <div class="w-10 h-10 rounded-xl overflow-hidden border border-slate-200/80 dark:border-slate-800/80 bg-slate-100 dark:bg-slate-900 flex items-center justify-center shrink-0">
                                @if($product->image)
                                    <img src="{{ route('products.image', $product->image) }}" class="w-full h-full object-cover" alt="{{ $product->name }}">
                                @else
                                    <span class="text-slate-400 dark:text-slate-600 text-[9px] font-black uppercase">N/A</span>
                        ...
                        <td class="table-td">
                            ...
                            <div class="text-[10px] text-slate-400 font-mono">SKU: {{ $product->sku ?? 'N/A' }}</div>
                        </td>
                        <td class="table-td">
                            <span class="font-black px-2.5 py-1 bg-slate-100 dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-800 rounded-xl text-[10px] text-slate-800 dark:text-slate-200 inline-block">
                                {{ $product->category->name ?? 'Sin Categoría' }}
                            </span>
                        </td>
                        <td class="table-td">
                            ...
                            <div class="relative inline-block" x-data="{ open: false }" @click.away="open = false">
                                <button @click="open = !open"
                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 rounded-xl text-xs font-bold transition-all cursor-pointer">
                                ...
                                <div x-show="open" x-transition class="absolute left-0 top-full mt-1 z-50 w-48 p-2.5 bg-[#0d121f] border border-slate-800 rounded-2xl shadow-xl space-y-1 text-xs">
                                    <p class="text-[10px] font-black uppercase text-slate-400 pb-1 border-b border-slate-800">Desglose por Talla</p>
                            ...
                            @else
                            <span class="text-xs text-slate-500 font-semibold">Sin tallas</span>
                            @endif
                        </td>
                        ...
                        <td class="table-td text-right space-x-1 whitespace-nowrap">
                            ...
                            <button @click="openDeleteModal({{ $product->id }})" class="p-2 text-rose-500 hover:bg-rose-500/10 rounded-xl transition-all cursor-pointer inline-flex items-center">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-16v1a3 3 0 003 3h10M9 3h6M4 7h16"/></svg>
                            </button>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="8" class="p-12 text-center text-slate-400 dark:text-slate-600 font-bold uppercase text-xs">No hay productos registrados.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="relative z-10 flex flex-col flex-1 min-h-0 bg-transparent" x-data="{ showSizes: false }">
            ...
            <div class="p-5 sm:p-7 space-y-4 overflow-y-auto flex-1 min-h-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div class="space-y-1.5">
                        <label class="form-label">Stock Inicial / Tallas</label>
                        <div class="flex items-center gap-2">
                            <input type="number" name="stock" x-model.number="globalStock" min="0" placeholder="Ej. 10" class="form-input" required>
                            <button type="button" @click="showSizes = !showSizes" class="px-3.5 py-2.5 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 rounded-xl text-xs font-black transition-all cursor-pointer shrink-0 flex items-center gap-1.5">
                ...
                <div class="space-y-2 pt-2 border-t border-slate-200/60 dark:border-slate-800/60" x-show="showSizes" x-transition>
                    <div class="flex items-center justify-between">
                        <label class="form-label text-indigo-400">Distribución por Talla</label>
                        <span class="text-[11px] font-bold" :class="totalTallasAssigned > globalStock ? 'text-rose-500' : 'text-slate-400'">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 max-h-48 overflow-y-auto p-1">
                        <template x-for="(item, index) in tallasList" :key="index">
                            <div class="flex items-center justify-between p-2.5 bg-slate-50 dark:bg-[#070a11] border border-slate-200 dark:border-slate-800 rounded-xl">
                                ...
                                <input type="number"
                                    ...
                                    placeholder="Pzas"
                                    class="w-16 text-center form-input !py-1 text-xs disabled:opacity-40" />
                            </div>
                        </template>
                    </div>
    ...
    <x-modal name="delete" title="¿Confirmar Destrucción?" maxWidth="max-w-sm" dotColor="bg-rose-500">
        <div class="p-6 text-center">
            ...
            <p class="text-[11px] text-slate-500 dark:text-slate-400 mb-6 font-semibold px-2 leading-relaxed">
                Esta acción eliminará de forma irreversible el producto y todo su desglose de tallas del servidor.
            </p>
