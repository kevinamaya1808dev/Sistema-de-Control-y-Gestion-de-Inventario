...
<div class="max-w-5xl mx-auto space-y-4 sm:space-y-6" x-data="categoryManagement()">
    ...
    @if(session('success'))
        <div class="bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 dark:text-emerald-400 p-3.5 sm:p-4 rounded-2xl shadow-xs mb-4 sm:mb-6 flex items-center space-x-3 transition-all">
            <div class="p-2 rounded-xl bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 flex-shrink-0">
    ...
    @if(session('error'))
        <div class="bg-rose-500/10 border border-rose-500/20 text-rose-700 dark:text-rose-400 p-3.5 sm:p-4 rounded-2xl shadow-xs mb-4 sm:mb-6 flex items-center space-x-3 transition-all">
            <div class="p-2 rounded-xl bg-rose-500/20 text-rose-600 dark:text-rose-400 flex-shrink-0">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 sm:pb-6 border-b border-slate-200/60 dark:border-slate-800/80">
        <div>
            <h1 class="text-xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                Gestión de Categorías
            </h1>
            <p class="text-xs sm:text-sm text-slate-400 dark:text-slate-500 mt-0.5 font-semibold tracking-wide">
                Administra las clasificaciones globales para el control de inventario
            </p>
        </div>
        
        <button @click="openCreateModal()" 
                class="w-full sm:w-auto px-5 py-3 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black rounded-xl shadow-xs hover:shadow-md transition-all flex items-center justify-center gap-2.5 cursor-pointer transform active:scale-98">
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xs border border-slate-200/70 dark:border-slate-800/80 overflow-hidden">
        
        <!-- Encabezado Interno -->
        <div class="px-4 sm:px-7 py-4 sm:py-5 bg-slate-50/50 dark:bg-slate-950/40 border-b border-slate-100 dark:border-slate-800/80">
            <h2 class="text-xs sm:text-sm font-black text-slate-800 dark:text-white uppercase tracking-wider">Categorías Activas</h2>
            <p class="text-[10px] sm:text-[11px] text-slate-400 dark:text-slate-500 font-bold mt-0.5">Listado maestro en tiempo real</p>
        </div>
        
        <!-- 1. VISTA TARJETAS (SOLO MÓVILES: < 768px) -->
            ...
            @forelse($categories as $category)
                <div class="p-4 space-y-3 hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <div class="flex items-center justify-between gap-2">
                        <span class="font-black text-slate-900 dark:text-white px-2.5 py-1 bg-slate-100 dark:bg-slate-800 border border-slate-200/50 dark:border-slate-700 rounded-xl text-xs tracking-wide">
                            {{ $category->name }}
                        </span>
                    ...
                    <div class="flex items-center justify-end gap-2 pt-2 border-t border-slate-100/60 dark:border-slate-800/40">
                        <button @click="openEditModal({{ $category->id }}, '{{ addslashes($category->name) }}', '{{ addslashes($category->description) }}')" 
                                class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold text-amber-600 dark:text-amber-400 bg-amber-500/10 rounded-lg border border-amber-500/20 active:scale-95 transition-all">
                        ...
                        <button @click="openDeleteModal({{ $category->id }})" 
                                class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold text-rose-600 dark:text-rose-400 bg-rose-500/10 rounded-lg border border-rose-500/20 active:scale-95 transition-all">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-16v1a3 3 0 003 3h10M9 3h6M4 7h16"/></svg>
                            Eliminar
                        </button>
                    </div>
                </div>
            @empty
                <div class="p-10 text-center text-slate-400 dark:text-slate-600 font-bold">
                    <div class="flex flex-col items-center justify-center space-y-2">
                        <div class="p-3 bg-slate-100 dark:bg-slate-950 rounded-2xl text-slate-300 dark:text-slate-700">
                            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0a2 2 0 01-2 2H6a2 2 0 01-2-2m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5M14 10h1"/></svg>
                        </div>
        ...
        <!-- 2. VISTA TABLA (TABLETS Y ESCRITORIO: >= 768px) -->
    ...
    <div x-show="modals.create" 
         ...
         style="display: none;">
        <div class="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200/40 dark:border-slate-800" @click.outside="modals.create = false">
            <div class="px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-950/20">
                ...
                <button type="button" @click="modals.create = false" class="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-400 flex items-center justify-center cursor-pointer transition-colors font-bold">✕</button>
            </div>
            <form action="{{ route('categories.store') }}" method="POST">
                ...
                <div class="p-5 sm:p-6 space-y-4 sm:space-y-5">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" class="w-full px-4 py-2.5 sm:py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:focus:border-indigo-500 text-slate-900 dark:text-white text-xs font-bold focus:outline-hidden focus:ring-4 focus:ring-indigo-500/10 transition-all" placeholder="Ej. Hardware" required>
                    </div>
                    <div>
                        ...
                        <textarea name="description" rows="3" class="w-full px-4 py-2.5 sm:py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:focus:border-indigo-500 text-slate-900 dark:text-white text-xs font-bold focus:outline-hidden focus:ring-4 focus:ring-indigo-500/10 transition-all" placeholder="Especifica qué tipo de artículos entran aquí..."></textarea>
                    </div>
                </div>
                <div class="px-5 sm:px-6 py-4 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-xs cursor-pointer transition-all">Guardar Categoría</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="modals.edit" 
         ...
         x-transition:leave-end="opacity-0 scale-95"
         style="display: none;">
        <div class="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-slate-200/40 dark:border-slate-800" @click.outside="modals.edit = false">
            <div class="px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-950/20">
                <h3 class="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                    <span class="w-2 h-2 rounded-full bg-amber-500"></span> Modificar Categoría
                </h3>
                <button type="button" @click="modals.edit = false" class="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 flex items-center justify-center cursor-pointer font-bold">✕</button>
            </div>
            <form :action="'{{ route('categories.index') }}/' + formEdit.id" method="POST">
                ...
                <div class="p-5 sm:p-6 space-y-4 sm:space-y-5">
                    <div>
                        ...
                        <input type="text" x-model="formEdit.name" name="name" class="w-full px-4 py-2.5 sm:py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:focus:border-indigo-500 text-slate-900 dark:text-white text-xs font-bold focus:outline-hidden focus:ring-4 focus:ring-indigo-500/10 transition-all" required>
                    </div>
                    <div>
                        ...
                        <textarea x-model="formEdit.description" name="description" rows="3" class="w-full px-4 py-2.5 sm:py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 focus:border-indigo-500 dark:border-slate-800 dark:focus:border-indigo-500 text-slate-900 dark:text-white text-xs font-bold focus:outline-hidden focus:ring-4 focus:ring-indigo-500/10 transition-all"></textarea>
                    </div>
                </div>
                <div class="px-5 sm:px-6 py-4 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 flex justify-end gap-3">
    ...
    <div x-show="modals.delete" 
         ...
         x-transition:leave-end="opacity-0 scale-95"
         style="display: none;">
        <div class="bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden text-center p-5 sm:p-6 border border-slate-200/40 dark:border-slate-800" @click.outside="modals.delete = false">
            ...
            <h3 class="text-sm font-black text-slate-900 dark:text-white mb-1.5 uppercase tracking-wide">¿Confirmar Eliminación?</h3>
            ...
            <form :action="'{{ route('categories.index') }}/' + formDelete.id" method="POST" class="flex gap-3">
                @csrf
                @method('DELETE')
                <button type="button" @click="modals.delete = false" class="w-1/2 py-2.5 sm:py-3 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 font-black text-xs cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">Abortar</button>
                <button type="submit" class="w-1/2 py-2.5 sm:py-3 bg-rose-600 hover:bg-rose-700 text-white font-black text-xs rounded-xl cursor-pointer transition-all shadow-xs">Eliminar</button>
            </form>
        </div>
    </div>
</div>
