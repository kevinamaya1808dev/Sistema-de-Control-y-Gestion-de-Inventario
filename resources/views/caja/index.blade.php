...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    {{-- Encabezado Estilo Stock / Inventario --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-black text-slate-800 dark:text-white tracking-tight">Movimientos de Caja</h1>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Gestión de apertura de turno, corte de caja y registro de flujo de efectivo.</p>
        </div>
        <div>
            @if($cajaActiva)
    ...
    @if(!$cajaActiva)
        {{-- FORMULARIO DE APERTURA (Estilo Tarjeta de Inventario) --}}
        <div class="max-w-md mx-auto bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
            ...
            <form action="{{ route('caja.abrir') }}" method="POST" class="p-6 space-y-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" step="0.01" min="0" name="monto_apertura" id="monto_apertura" required placeholder="0.00"
                            class="pl-8 w-full rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 focus:border-indigo-500 focus:ring-indigo-500 text-slate-800 dark:text-white font-bold text-base">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="observaciones" id="observaciones" rows="3" placeholder="Opcional..."
                        class="w-full rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 focus:border-indigo-500 focus:ring-indigo-500 text-xs text-slate-700 dark:text-slate-300"></textarea>
                </div>

                <button type="submit" class="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-semibold rounded-xl transition duration-150 shadow-sm flex items-center justify-center gap-2 text-xs cursor-pointer">
    ...
    @else
        {{-- LAYOUT PRINCIPAL DE 2 COLUMNAS (Estilo Panel de Control / Stock) --}}
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div class="p-5 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/50 flex items-center justify-between">
                        <h2 class="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider">Resumen de Turno</h2>
                    ...
                    <div class="p-5 space-y-3 text-xs">
                        ...
                        <div class="flex justify-between text-slate-500 dark:text-slate-400">
                            <span>Apertura:</span>
                            <span class="font-medium text-slate-800 dark:text-white">{{ optional($cajaActiva->fecha_apertura)->format('d/m/Y') }}</span>
                        </div>
                    ...
                    <form id="formCorteCaja" action="{{ route('caja.cerrar') }}" method="POST" class="p-5 border-t border-slate-100 dark:border-slate-800 space-y-4 bg-slate-50/30 dark:bg-slate-950/20">
                        @csrf
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" step="0.01" min="0" name="monto_cierre" id="monto_cierre" required placeholder="0.00"
                                    class="pl-8 w-full rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 focus:border-rose-500 focus:ring-rose-500 text-slate-800 dark:text-white font-bold text-sm">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="observaciones" id="observaciones_cierre" rows="2" placeholder="Sin novedades..."
                                class="w-full rounded-xl border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 focus:border-rose-500 focus:ring-rose-500 text-xs text-slate-700 dark:text-slate-300"></textarea>
                        </div>

                        <button type="button" onclick="confirmarCorteCaja()"
                            class="w-full py-2.5 px-4 bg-rose-600 hover:bg-rose-700 active:bg-rose-800 text-white font-semibold rounded-xl transition duration-150 shadow-sm flex items-center justify-center gap-2 text-xs cursor-pointer">
            ...
            <div class="lg:col-span-8 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div class="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-950/50">
                        <div class="flex items-center gap-2">
                            ...
                            <h2 class="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider">Ventas del Turno</h2>
                        </div>
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                @forelse($ventasTurno ?? [] as $venta)
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition duration-150">
                                        <td class="px-5 py-3.5 whitespace-nowrap">{{ $venta->created_at ? $venta->created_at->format('d/m/Y') : '' }}</td>
                ...
                {{-- TABLA DE GASTOS Y SALIDAS --}}
                <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
                    <div class="p-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-950/50">
                        <div class="flex items-center gap-2">
                            <span class="w-2.5 h-2.5 rounded-full bg-rose-500"></span>
                            <h2 class="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider">Gastos y Salidas</h2>
                        </div>
                        <span class="text-xs font-bold text-slate-600 dark:text-slate-300 bg-slate-200/50 dark:bg-slate-800 px-2.5 py-1 rounded-lg">
                        ...
                        <table class="w-full text-left text-xs text-slate-600 dark:text-slate-300">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                @forelse($gastosTurno ?? [] as $gasto)
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition duration-150">
                                        <td class="px-5 py-3.5 whitespace-nowrap">{{ $gasto->created_at ? $gasto->created_at->format('d/m/Y') : '' }}</td>
