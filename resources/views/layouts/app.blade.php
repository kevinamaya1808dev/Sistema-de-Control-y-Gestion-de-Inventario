...
<body class="h-screen w-full max-w-full overflow-x-hidden font-sans antialiased text-slate-800 dark:text-slate-100 bg-slate-100 dark:bg-[#070a11] transition-colors duration-200">
    {{-- Estado global con Alpine.js --}}
    <div x-data="{ sidebarOpen: false, sidebarCollapsed: false }" class="h-screen flex w-full max-w-full overflow-hidden bg-slate-100 dark:bg-[#0b0f19]">
        ...
        <aside :class="{
                ...
                class="fixed lg:static inset-y-0 left-0 z-50 w-72 bg-white dark:bg-[#0e1322] text-slate-600 dark:text-slate-300 flex-shrink-0 flex flex-col border-r border-slate-200 dark:border-slate-800/80 h-screen transition-all duration-300 ease-in-out">
            ...
            <nav class="flex-1 px-4 py-3 space-y-4 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">

                {{-- GENERAL --}}
                <div class="space-y-1">
                    <p class="px-3 text-[11px] font-extrabold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-1.5">General</p>
                ...
                @can('manage-users')
                    <div class="space-y-1">
                        <p class="px-3 text-[11px] font-extrabold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-1.5">Administración</p>
        ...
        <!-- ÁREA DERECHA: HEADER FIJO + MAIN SCROLLEABLE -->
        <div class="flex-1 flex flex-col min-w-0 max-w-full h-screen overflow-hidden">
            
            {{-- HEADER FIJO --}}
            <header class="h-20 bg-white dark:bg-[#0b0f19] border-b border-slate-200 dark:border-slate-800/80 px-4 md:px-8 flex items-center justify-between shadow-xs z-10 w-full min-w-0 flex-shrink-0 transition-colors duration-200">
                
                <div class="flex items-center gap-3 min-w-0">
                    <button @click="sidebarOpen = !sidebarOpen" class="lg:hidden text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white p-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 flex-shrink-0">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
                    </button>

                    <button @click="sidebarCollapsed = !sidebarCollapsed" class="hidden lg:flex items-center justify-center p-2 rounded-xl text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800/60 flex-shrink-0">
